...
def weather():
    ...
    if request.method == 'POST':
        ...
        print("Geocoding response:", geocoding_response.status_code, geocoding_response.json()) 
        ...
        if geocoding_response.status_code == 200 and geocoding_response.json().get('results'):
            ...
            print("Coordinates found:", latitude, longitude)  # Verify coordinates

            # Fetch weather data with additional attributes
            weather_response = requests.get(
               f'https://api.open-meteo.com/v1/forecast?latitude={latitude}&longitude={longitude}&hourly=temperature_2m,windspeed_10m,precipitation&timezone=auto'
            )

            print("Weather response:", weather_response.status_code, weather_response.json())  # Check weather response
